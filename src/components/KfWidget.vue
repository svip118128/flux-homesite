...
onMounted(() => {
  ...
  pushMsg('system', '', 'kf.intro'); // Initial welcome message with key
  connectWS();

  // Watch locale change to update welcome message or reconnect if needed?
  // Actually the original widget updated text. Here Vue does it reactively for UI.
  // For the system message, it's pushed once. We might want to clear and push new one if lang changes? 
  ...
  <div id="jd-kf-widget" :class="{ 'drag-active': isDragActive }"
    ...
    <div class="jd-window" v-show="isOpen">
      ...
      <div class="jd-actions">
        <!-- We use <a> for external links, router-link for internal if needed. 
             Original widget logic for telegram link is complex, handled in tgLink() -->
        ...
        <router-link class="jd-btn-link" :to="docsUrl">{{ $t('kf.docs') }}</router-link>
        <router-link class="jd-btn-link" :to="faqUrl">{{ $t('kf.faq') }}</router-link>
      </div>
